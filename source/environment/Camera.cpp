...
auto environment::Camera::update() -> void
{
	...
	/*if (this->lockToGlider)
	{
		...
		// Rotate the camera position around origin equal to glider
		this->pos = glm::vec3(glider->getRotQuat() * glm::vec4(this->pos, 0.1f));

		// Rotate the camera's up around origin equal to glider
		this->up = glm::vec3(glider->getRotQuat() * glm::vec4(0.0f, 1.0f, 0.0f, 0.1f));

		// Shift rotated camArm position to position behind the plane
		this->pos += glider->getPos();

		// Set the looking direction ("target" - vector) to norm(diff in positions)
...
glm::mat4 environment::Camera::getPerspectiveMatrix(){
	return glm::perspective(glm::radians(this->fov), windowSize().x / windowSize().y, 0.1f, -10.0f);	
}
...
void environment::Camera::rotateBy(float angleX, float angleY)
{
	...
	//this->up = (glm::vec3) (rotationVertMatrix * glm::vec4(this->up, 0)); 
...
{
	this->fov = glm::clamp(this->fov += ((in) ? -1.0f : 1.0f), 15.0f, 60.0f);
}